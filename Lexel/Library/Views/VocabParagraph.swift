...
struct VocabParagraph: View {
    ...
    @State private var selectedWord: LexelToken? = nil
...
//        self.triggerTranslation()  // for ios 18, once i can get this to work
        ...
        WordView(word: token.lemma, displayWord: token.rawValue, showFamiliarityHighlight: (index != selectedWordIndex || paragraph != selectedParagraphIndex))
            ...
            .popover(isPresented: self.makeIsPresented(wordIndex: index, paragraphIndex: paragraph)) {
                if let definition = translatedWord {
                    FamiliarWordView(token: selectedWord!, language: story.language.bcp47, definition: definition)
                } else {
                    ProgressView()
                }
            }
